...
from saleor_app.workers.errors import NonTransientError, TransientError, UnrecognizedEventPayload
...
class SaleorAppSQSWorker(SaleorAppWorker):
    ...
    def __init__(self, queue_name: str, max_number_of_messages: int = 1, wait_time_seconds: int = 0, *args, **kwargs):
    ...
    async def process_message(self, message):  # TODO: figure out the type of a message and mark that as the return type
        try:
            webhook = self.parse_webhook_payload(message_body=message.body)
        except ValidationError:
            raise UnrecognizedEventPayload("The message body was not recognized", message=message)
        
        # TODO: allow to decide if not recognized events should be ignored or dead lettered
        handler = self.handlers.get()  # TODO: figure out how to get an event_type from a message, is it in the message? if not add to webhook meta?
    ...
    async def handle_transient_error(self, message, exc):
        while True:
            logger.warning("Transient error occured halting, will retry again in %s seconds", self.transient_error_wait_seconds, exc_info=exc)
    ...
    async def handle_unhandled_exceptions(self, exc):
        ...
        logger.critical("Failed to handle a message, message will be put in SQS deadletter", exc_info=exc)
        return
    ...
    async def loop(self):
        ...
        while True:
            ...
            results: List[Union[Any, Exception]] = asyncio.gather(*coros, return_exceptions=True)  # TODO: figure out the type of a message and put it instead of Any
            successful_results = []
